...
// √лобальные / file-scope переменные
Net* net_handl = nullptr;
EightbitState* g_eightbit = nullptr;

// ‘лаг говорит, был ли успешно создан/включЄн detour на SV_BroadcastVoice
...
extern "C" void hook_BroadcastVoiceData(IClient * cl, int nBytes, char* data, int64 xuid) {
	...
	// Ѕезопасность: если хук почему-то вызван, но флаг не установлен Ч возвращаемс€ в транзит.
...
GMOD_MODULE_OPEN()
{
	...
	if (sv_bcast == nullptr) {
		std::cerr << "[eightbit] Could not locate SV_BroadcastVoice symbol Ч voice hook disabled. Module will run in degraded mode.\n";
		...
		try {
			...
			std::cerr << "[eightbit] Exception while creating/enabling detour Ч voice hook disabled.\n";
			voiceHookAvailable = false;
		}
	}
